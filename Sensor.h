...
// public:
//
//   InteractionChannel(Stream &_stream) : stream(_stream) { }
//
// 	InteractionChannel(Stream &_stream,char (*getKey)()) : stream(_stream) {
// 		this->readFunction = getKey;
// 	}
//
// 	InteractionChannel(Stream &_stream,char (*getKey)(),void (*writeChar)(String s)) : stream(_stream) {
// 		this->readFunction = getKey;
...
class Sensor
{
	public:
		...
		int streamAdd(Stream &stream,char (*getKey)());
		int streamAdd(Stream &stream,void (*writeChar)(String s));
		int streamAdd(char (*getKey)(),void (*writeChar)(String s));
	...
	private:
		...
		InteractionChannel* streams[MAX_IO_STREAMS];
		// InteractionChannel* defaultStream;
		/* Add to the redings history. If the buffer is full rotate */
		void pushLastReadings(float value);
		int streamPush(InteractionChannel* ioChannel);
};
